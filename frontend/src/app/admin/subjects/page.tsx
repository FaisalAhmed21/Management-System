          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-800/50 border-b border-gray-800">
                <th className="p-4 text-sm font-medium text-gray-400">Subject Name</th>
            ...
            <tbody className="divide-y divide-gray-800">
              {subjects.map((sub) => (
                <tr key={sub.id} className="hover:bg-gray-800/20 transition-colors">
                  <td className="p-4 text-sm text-gray-200">{sub.name}</td>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-md p-6 shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-400 mb-1">Class/Course</label>
                <select required value={formData.classCourseId} onChange={(e) => setFormData({ ...formData, classCourseId: e.target.value })} className="w-full bg-gray-950 border border-gray-800 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-blue-500">
